...
class CappuccinoXL(Recipe):

    def __init__(self) -> None:
        self.__cup = Cup(400)

    def __str__(self) -> str:
        return f'Cappuccino XL'

    def add_base_component(self) -> None:
        milk = Frother(RegularMilk(360.0)).pass_milk(FrothedMilk())
        self.__cup.add_ingredient(milk)

    def add_second_component(self) -> None:
        grounded_coffee = Grinder(RoastedCoffee(15.0)).grind()
        infuser = Infuser(grounded_coffee).make_coffee(Espresso, 40.0)
