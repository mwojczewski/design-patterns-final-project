...
class CafeLatte(Recipe):
    ...
    def add_base_component(self) -> None:
        milk = Frother(RegularMilk(50.0)).pass_milk(RegularMilk())
        self.__cup.add_ingredient(milk)

    def add_second_component(self) -> None:
        milk = Frother(RegularMilk(80.0)).pass_milk(FrothedMilk())
        self.__cup.add_ingredient(milk)

    def add_top_component(self) -> None:
        grounded_coffee = Grinder(RoastedCoffee(10.0)).grind()
        infuser = Infuser(grounded_coffee).make_coffee(Espresso, 70.0)
