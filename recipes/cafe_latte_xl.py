...
class CafeLatteXL(Recipe):

    def __init__(self) -> None:
        self.__cup = Cup(400)

    def __str__(self) -> str:
        return f'Cafe Latte XL'

    def add_base_component(self) -> None:
        milk = Frother(RegularMilk(100.0)).pass_milk(RegularMilk())
        self.__cup.add_ingredient(milk)

    def add_second_component(self) -> None:
        milk = Frother(RegularMilk(160.0)).pass_milk(FrothedMilk())
        self.__cup.add_ingredient(milk)

    def add_top_component(self) -> None:
        grounded_coffee = Grinder(RoastedCoffee(20.0)).grind()
        infuser = Infuser(grounded_coffee).make_coffee(Espresso, 140.0)
